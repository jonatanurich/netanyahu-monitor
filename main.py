...
BUILD = "2026-08-06.duplicate-fix"
...
# "שרה" לבדה היא גם שם נפוץ וגם פועל. היא נספרת רק כשמופיע לצידה
# הקשר שמצביע על רעיית ראש הממשלה.
SARA_LOOSE_RE = _build_re(("שרה", "שרי"))
SARA_CONTEXT_RE = _build_re(
    (
        "נתניהו", "ביבי", "רעיית ראש הממשלה", "אשת ראש הממשלה",
        "הגברת הראשונה", "בלפור", "רעייתו",
    )
)
...
def detect_subjects(text: str) -> list[str]:
    ...
    if (
        "sara" not in subjects
        and SARA_LOOSE_RE is not None
        and SARA_LOOSE_RE.search(text)
        and SARA_CONTEXT_RE is not None
        and SARA_CONTEXT_RE.search(text)
    ):
        subjects.insert(0, "sara")
...
def _sender_body() -> None:
    ...
    while True:
        try:
            row = db().execute(
                """
                SELECT id, body, attempts, target FROM outbox
