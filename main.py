...
CLUSTER_WINDOW_MINUTES = env_int("CLUSTER_WINDOW_MINUTES", 45, 2, 240)
...
CLUSTER_CONTAINMENT = env_float("CLUSTER_CONTAINMENT", 0.6, 0.3, 1.0)
CLUSTER_MIN_CONTAINED_TOKENS = env_int("CLUSTER_MIN_CONTAINED_TOKENS", 3, 2, 15)
CLUSTER_CONTAINMENT_STRICT = env_float("CLUSTER_CONTAINMENT_STRICT", 0.8, 0.4, 1.0)
# רצפה שמונעת מאשכול גדול לבלוע פריטים שרק במקרה חולקים מילים
CLUSTER_JACCARD_FLOOR = env_float("CLUSTER_JACCARD_FLOOR", 0.15, 0.05, 0.5)
# תקרת טוקנים לאשכול — האשכול צובר מילים ממקורות שמצטרפים אליו
CLUSTER_MAX_TOKENS = env_int("CLUSTER_MAX_TOKENS", 45, 10, 200)
...
def fetch_telegram(channel: str) -> list[dict[str, str]]:
    ...
    try:
        ...
        for wrap in selected:
            ...
            message = {"text": text, "link": link, "source": channel}
...
def process_scan() -> tuple[int, int]:
    ...
    candidates: list[
        tuple[str, str, str, str, str, str, list[str], bool, bool, bool]
    ] = []
    ...
    for (item_key, key, display, text, link, kind, subjects, is_statement,
         always_alert, is_primary) in candidates:
...
def build_digest(now: datetime) -> str | None:
    ...
    rows = db().execute(
        ...
        SELECT c.id, c.sample_text, c.sources, c.source_count, c.kind,
               c.tone, c.score, c.pushed
        FROM clusters c
        WHERE c.created_at_utc >= ?
        ORDER BY c.score DESC, c.source_count DESC, c.id DESC
        LIMIT 25
        """,
        (since,),
    ).fetchall()
    ...
    for index, row in enumerate(rows, start=1):
        ...
        seen_mark = "" if int(row["pushed"]) else " 🆕"
        lines.append(
            f"\n{score_badge(int(row['score']))} *{index}.* {marker}{badge}"
            f"{seen_mark} {clean_text(str(row['sample_text']))[:200]}"
            f"\n   📡 {row['source_count']} מקורות: {', '.join(source_list[:5])}"
        )
    lines.append(
        f"\n🆕 = לא נשלח כהתראה בזמן אמת (מתחת לסף {push_threshold()})"
        f"\n⏰ {now.strftime('%H:%M')}"
    )
    return "\n".join(lines)
