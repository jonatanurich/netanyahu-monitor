...
HTTP.headers.update(
    {
        "User-Agent": (
            "Mozilla/5.0 (compatible; NetanyahuMediaMonitor/9.0; "
            "+https://github.com/)"
        )
    }
)
...
def fetch_telegram(channel: str) -> list[dict[str, str]]:
    ...
    try:
        ...
        if len(wraps) and len(selected) == len(wraps) >= TELEGRAM_MESSAGES_PER_SCAN:
            LOGGER.warning(
                "Telegram channel %s returned the full window (%s). "
                "Consider a shorter SCAN_INTERVAL_SECONDS.",
                channel,
                len(wraps),
            )
...
def health():
    ...
    healthy = monitor_alive and sender_alive and not stale and not failing
    payload = {
        "ok": healthy,
        ...
        "outbox_pending": pending_outbox_count(),
